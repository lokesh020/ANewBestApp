...
import {SectionList, Text,Platform} from 'react-native';
...
import sectionListGetItemLayout from './SectionGetItemLayout'
...
class AgendaList extends Component {
  ...
  scrollToDate(dateStr){
      const sectionIndex = this.getSectionIndex(dateStr);
      if (commons.isAndroid) {
        this.scrollToSectionWithoutAnimation(sectionIndex);
      }else{
        this.scrollToSection(sectionIndex)
      }
  }

  scrollToSectionWithoutAnimation(sectionIndex) {
  ...
  render() {
    return (
      <SectionList
        ...
        onScrollToIndexFailed={(info) => { console.log('onScrollToIndexFailed info: ', info); }}
        getItemLayout={ commons.isAndroid ? sectionListGetItemLayout({
          // The height of the row with rowData at the given sectionIndex and rowIndex
          getItemHeight: (data, sectionIndex, rowIndex) => 60,
          // These four properties are optional
          getSectionHeaderHeight: () => 40, // The height of your section headers
        }) : undefined
      } // onViewableItemsChanged is not updated when list scrolls!!!
      />
    );
  }
}
