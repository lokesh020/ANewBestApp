...
function ActivityStartScreen({ navigation, route }) {

    const { activityId, subCategoryId, fromWhere, fromWhichTabSelection, activityLogId, completedTime } = route.params;
    ...
    const onMediaItemPress = (item) => {
        ...
        } else if (item.mediaName.toLowerCase() == "pdf"){
            pdfReaderModelRef.current.show()
        }else{
            UtilityFunc.showAlertWithSingleAction("work in progress", () => { })
        }
    }

    /**
     * This action navigates to Timer screen with attachment detail(video, audio and pdf)
     */
    const onMediaItemStartButtonPress = (item) => {
        navigateToTimerScreenWithAttachment(activityDetailModel.id, activityDetailModel.wellnessSubCategoryId, item.mediaName.toLowerCase(), item.fileUrl, activityDetailModel.duration)
    }

    const navigateToTimerScreenWithAttachment = (activityId, subCategoryId, mediaType, mediaUrl, duration) => {
        navigation.navigate("TimerScreen",{ fromWhere: "ActivityStartScreen", fromWhichTabSelection ,activityId, subCategoryId, mediaType, mediaUrl, completedTime, activityLogId, duration })
    }

    const navigateToTimerScreenWithOutAttachment = (activityId, subCategoryId, item) => {
        ...
        return (
            <KeyboardAwareScrollView style={{ flex: 1 }} bounces={false} showsVerticalScrollIndicator={false}>
                <View style={styles.container}>
                    ...
                    <MediaList
                        data={activityDetailModel.media}
                        onItemPress={onMediaItemPress}
                        onStartButtonPress={onMediaItemStartButtonPress}
                        fromWhichTabSelection={fromWhichTabSelection}
                    />
    ...
    return (
        <SafeAreaView style={{ backgroundColor: Colors.BG_ACTIVITY_SCREEN, flex: 1 }} edges={['top']}>
            ...
            <PdfReaderModal ref={pdfReaderModelRef} resource={{ uri : (selectedMediaItem && selectedMediaItem.fileUrl)}} onCancel={onPdfReaderModalCancel} />
        </SafeAreaView >
    )
}
...
const MediaList = ({ data = [], onItemPress = () => { }, onStartButtonPress = () => { }, fromWhichTabSelection = "" }) => {
    ...
    return (
        <View style={{ display: 'flex', backgroundColor: 'red' }}>
            {data.map((element, index) => (<MediaItem key={index.toString()}
                fromWhichTabSelection={fromWhichTabSelection}
                item={element} index={index}
                onPress={() => onItemPress(element, index)}
                onStartNextBtnPress={() => onStartButtonPress(element, index)}
            />))}
        </View>
    )
...
const MediaItem = ({ item, onPress, onStartNextBtnPress, fromWhichTabSelection }) => {
    let iconSrc = item.getMediaIconSrc()
    let txtStartNextButton = fromWhichTabSelection == "Planned" ? "Start" : "Next"
    return (
        <Pressable style={styles.mediaContainer} onPress={onPress}>
            ...
            <Pressable style={styles.startNextButton} onPress={onStartNextBtnPress}>
                <Text style={styles.startNextButtonText}>{txtStartNextButton}</Text>
            </Pressable>
        </Pressable>
    )
}
